...
class Volume(object):
    ...
    def __init__(self, fileinfo, image_size=None, image_resolution=None):
        ...
        '''
        self.filepath   = fileinfo['image']
        self.maskpath   = fileinfo['mask']
        ...
        self.suffix     = '_' + fileinfo['suffix'] if 'suffix' in fileinfo.keys() else ''
        self.frame_suffix = '_' + self.phase + self.suffix
        self.image_size = image_size
        self.image_resolution = image_resolution
        self.slices  = []
        self.slices_index = {}
        self.current = 0
        self.shape = nib.load(self.filepath).get_data().shape
    ...
    def process_slices(self):
        ...
        self.img_dat = nimg.get_fdata(), nimg.affine, nimg.header
        img = self.img_dat[0].copy()
        img = image_utils.normalise_image(img, 0, 1)

        if self.maskpath != '':
            nimg = nib.load(self.maskpath)
            self.mask_dat = nimg.get_fdata(), nimg.affine, nimg.header
...
class Dataset(object):
    ...
    def process_volumes(self):
        ...
        for f in self.files:
            ...
            self.volumes_index[f['image'].split('/')[-1].split('.')[0]] = len(self.volumes)-1
